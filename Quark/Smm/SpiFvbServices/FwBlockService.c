...
Arguments:
  FlashSize            Size in bytes of the flash device
...
--*/
{
  ...
  //
  Status = PcdSet32S (PcdSpiFlashDeviceSize, FlashSize);
...
--*/
{
  ...
  if (mInSmmMode == 0) { // !(EfiInManagementInterrupt ())) {
    WriteAddress &= mFvbModuleGlobal->SpiProtocol->FlashProtocol.FlashSize - 1;
    Status = mFvbModuleGlobal->SpiProtocol->FlashProtocol.WriteData (
                                            &mFvbModuleGlobal->SpiProtocol->FlashProtocol,
                                            WriteAddress,
                                            (UINT32) (*NumBytes),
                                            Buffer
                                            );
  } else {
    WriteAddress &= mFvbModuleGlobal->SmmSpiProtocol->FlashProtocol.FlashSize - 1;
    Status = mFvbModuleGlobal->SmmSpiProtocol->FlashProtocol.WriteData (
                                            &mFvbModuleGlobal->SmmSpiProtocol->FlashProtocol,
                                            WriteAddress,
                                            (UINT32) (*NumBytes),
                                            Buffer
                                            );
...
--*/
{
  ...
  WriteAddress += mFvbModuleGlobal->SpiProtocol->FlashProtocol.FlashSize
                - FLASH_SIZE;
  ASSERT ((SPI_ERASE_SECTOR_SIZE & (SPI_ERASE_SECTOR_SIZE - 1)) == 0); // Power of 2?
  ASSERT (SPI_ERASE_SECTOR_SIZE >= SIZE_4KB);
  BlockCount = ((WriteAddress & (SPI_ERASE_SECTOR_SIZE - 1)) + NumBytes
             + SPI_ERASE_SECTOR_SIZE - 1) / SIZE_4KB;
  if (mInSmmMode == 0 ) { // !(EfiInManagementInterrupt ())) {
    Status = mFvbModuleGlobal->SpiProtocol->FlashProtocol.Erase (
                                            &mFvbModuleGlobal->SpiProtocol->FlashProtocol,
                                            WriteAddress,           // Address
                                            BlockCount              // Blocks
                                            );
  } else {
    Status = mFvbModuleGlobal->SmmSpiProtocol->FlashProtocol.Erase (
                                            &mFvbModuleGlobal->SmmSpiProtocol->FlashProtocol,
                                            WriteAddress,           // Address
                                            BlockCount              // Blocks
                                            );
...
DEBUG((EFI_D_ERROR, "SmmSpiInit: Calling SmmLocateProtocol\n"));
    Status = mSmst->SmmLocateProtocol (&gEfiLegacySpiSmmFlashProtocolGuid, NULL, (VOID **) &mFvbModuleGlobal->SmmSpiProtocol);
    ASSERT_EFI_ERROR(Status);
  }

  SpiStatus = 0;
DEBUG((EFI_D_ERROR, "0x%08x: mFvbModuleGlobal\n", mFvbModuleGlobal));
DEBUG((EFI_D_ERROR, "0x%08x: mFvbModuleGlobal->SpiProtocol\n", mFvbModuleGlobal->SpiProtocol));
DEBUG((EFI_D_ERROR, "0x%08x: mFvbModuleGlobal->SpiProtocol->FlashProtocol.WriteStatus\n", mFvbModuleGlobal->SpiProtocol->FlashProtocol.WriteStatus));
DEBUG((EFI_D_ERROR, "SmmSpiInit: Calling FlashProtocol.WriteStatus\n"));
  Status = mFvbModuleGlobal->SmmSpiProtocol->FlashProtocol.WriteStatus (
                               &mFvbModuleGlobal->SmmSpiProtocol->FlashProtocol,
                               sizeof(SpiStatus),
                               &SpiStatus
                               );
  PublishFlashDeviceInfo (
              mFvbModuleGlobal->SmmSpiProtocol->FlashProtocol.FlashSize
              );
  return Status;
}
...
DEBUG((EFI_D_ERROR, "Calling GetFullDriverPath\n"));
  GetFullDriverPath (ImageHandle, SystemTable, &CompleteFilePath);

DEBUG((EFI_D_ERROR, "Calling EfiGetSystemConfigurationTable\n"));
...
DEBUG((EFI_D_ERROR, "0x%08x: mSmmBase2\n", mSmmBase2));
DEBUG((EFI_D_ERROR, "Calling InSmm\n"));
    mSmmBase2->InSmm (mSmmBase2, &InSmm);
DEBUG((EFI_D_ERROR, "Calling GetSmstLocation\n"));
  ...
  if (!InSmm) {
    mInSmmMode = 0;
    //
    // Obtain the legacy SPI flash protocol
    //
    Status = gBS->LocateProtocol (&gEfiLegacySpiFlashProtocolGuid, NULL,
                                  (VOID **) &mFvbModuleGlobal->SpiProtocol);
    ASSERT_EFI_ERROR (Status);

    PublishFlashDeviceInfo (
              mFvbModuleGlobal->SpiProtocol->FlashProtocol.FlashSize
              );
    ...
    //   1 for SST parts
    SpiStatus = 0;
    Status = mFvbModuleGlobal->SpiProtocol->FlashProtocol.WriteStatus (
                                 &mFvbModuleGlobal->SpiProtocol->FlashProtocol,
                                 sizeof(SpiStatus),
                                 &SpiStatus
                                 );
...
DEBUG((EFI_D_ERROR, "Calling SmmLocateProtocol\n"));
    Status = mSmst->SmmLocateProtocol (&gEfiLegacySpiSmmFlashProtocolGuid, NULL, (VOID **) &mFvbModuleGlobal->SmmSpiProtocol);
    if (EFI_ERROR(Status)) {
      Registration = NULL;
DEBUG((EFI_D_ERROR, "Calling SmmRegisterProtocolNotify\n"));
...
DEBUG((EFI_D_ERROR, "Calling SmmSpiInit\n"));
      Status  = SmmSpiInit();
    }
  }
  ...
  FwVolHeader           = NULL;
  do {
DEBUG((EFI_D_ERROR, "Calling GetFvbHeader\n"));
...
DEBUG((EFI_D_ERROR, "Calling EfiInitializeLock\n"));
...
DEBUG((EFI_D_ERROR, "Calling FlashFdErase\n"));
        Status = FlashFdErase (
...
DEBUG((EFI_D_ERROR, "Calling FlashFdWrite\n"));
        Status = FlashFdWrite (
                  (UINTN) FwhInstance->FvWriteBase[0],
                  (UINTN) BaseAddress,
                  &HeaderLength,
    ...
    //
DEBUG((EFI_D_ERROR, "Calling AllocateRuntimeCopyPool\n"));
...
DEBUG((EFI_D_ERROR, "Calling CopyGuid\n"));
      CopyGuid (
    ...
    if (!InSmm) {
      ...
      FwbHandle = NULL;
DEBUG((EFI_D_ERROR, "Calling SmmInstallProtocolInterface\n"));
  ...
  if (!InSmm) {
    ...
    //
    Handle = NULL;
DEBUG((EFI_D_ERROR, "Calling InstallProtocolInterface\n"));
