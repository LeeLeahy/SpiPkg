...
EFI_GUID *gLegacySpiControllerProtocolGuid = &gEfiLegacySpiSmmControllerProtocolGuid;
EFI_GUID *gSpiConfigurationProtocolGuid = &gEfiSpiSmmConfigurationProtocolGuid;
EFI_GUID *gSpiHcProtocolGuid = &gEfiSpiSmmHcProtocolGuid;
EFI_GUID gSpiBusLayerGuid =
{0xf31bb793, 0x2888, 0x433a, {0x83, 0x02, 0x17, 0x29, 0xb8, 0xa0, 0xef, 0x72}};
...
/**
  Display the device path for the host controller.
...
{
  DEBUG ((EFI_D_INFO, "SPI Host Controller\n"));
  DEBUG ((EFI_D_INFO, "  | Path: SMM\n"));
  
  //
  // Connect the host controller to the SPI bus on the board
  //
  SpiBus->BusConfig = gSpiConfigurationProtocol->BusList[0];
  if (SpiBus->BusConfig->FriendlyName != NULL) {
    DEBUG ((EFI_D_INFO, "  | Name: %s\n", SpiBus->BusConfig->FriendlyName));
  }
  return EFI_SUCCESS;
}
...
{
  return gSmst->SmmLocateProtocol (Protocol, NULL, Interface);
}
  ...
  )
{
  ...
  if (EFI_ERROR(Status)) {
    ...
    //
    Status = SpiBusStartup (Handle, SpiHcProtocol);
  }
  return Status;
}
  ...
  )
{
  ...
  ASSERT(gSpiConfigurationProtocol->BusCount == 1);

  //
  // Start a SPI bus for each SPI host controller that is installed
  //
  Status = gSmst->SmmRegisterProtocolNotify (gSpiHcProtocolGuid,
                                             SpiBusSpiHcProtocolAvailable,
                                             &gSpiHcProtocolRegistration
                                             );
  if (EFI_ERROR(Status)) {
    DEBUG((EFI_D_ERROR, "SpiBus: Failed to register host controller notify!, Status: %r\n", Status));
  }
  return Status;
}
  ...
  )
{
  ...
  if (EFI_ERROR(Status)) {
    DEBUG((EFI_D_ERROR, "SpiBus: Failed to register configuration database notify!, Status: %r\n", Status));
  }
  return Status;
}
